...
import { CONFIG_DIR_NAME, getAgentDir, isBunBinary, resolveProjectRoot } from "../../config.ts";
...
export async function discoverAndLoadExtensions(
	...
	// 1. Project-local extensions: cwd/${CONFIG_DIR_NAME}/extensions/, or the git
	// worktree root's ${CONFIG_DIR_NAME}/extensions/ when cwd has no local one.
	const localExtDir = path.join(resolveProjectRoot(resolvedCwd), CONFIG_DIR_NAME, "extensions");
